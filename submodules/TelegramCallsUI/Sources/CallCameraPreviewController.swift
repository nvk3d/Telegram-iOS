...
protocol CallPreviewVideoNode: ASDisplayNode {
...
final class CallCameraPreviewController: ViewController {
    private var controllerNode: CallCameraPreviewControllerNode {
        return self.displayNode as! CallCameraPreviewControllerNode
    }
    ...
    private let requestEndFrame: () -> CGRect?
    private let shareCamera: (ASDisplayNode, Bool) -> Void
    private let switchCamera: () -> Void
    private let cancel: () -> Void
    
    private var presentationDataDisposable: Disposable?
    private var cameraNodeDisposable: Disposable?
    
    init(
        fromRect: CGRect,
        sharedContext: SharedAccountContext,
        cameraNode: CallPreviewVideoNode,
        requestEndFrame: @escaping () -> CGRect?,
        ...
        self.requestEndFrame = requestEndFrame
        ...
        cameraNodeDisposable = cameraNode.ready.start { value in
            Queue.mainQueue().after(0.4) { [weak self] in self?._ready.set(.single(value)) }
        }
    ...
    deinit {
        ...
        self.cameraNodeDisposable?.dispose()
    }
    
    override public func loadDisplayNode() {
        self.displayNode = CallCameraPreviewControllerNode(controller: self, fromRect: fromRect, sharedContext: self.sharedContext, cameraNode: self.cameraNode)
        self.controllerNode.requestEndFrame = { [weak self] in
            self?.requestEndFrame() ?? .zero
        }
...
private class CallCameraPreviewControllerNode: ViewControllerTracingNode, UIScrollViewDelegate {
    private weak var controller: CallCameraPreviewController?
    private let fromRect: CGRect
    private let sharedContext: SharedAccountContext
    ...
    private var isCancelPressed: Bool = false
    ...
    init(
        ...
        self.doneButton.title = "Start Video" // self.presentationData.strings.VoiceChat_VideoPreviewContinue
        ...
        self.previewContainerNode.backgroundColor = .clear //UIColor(rgb: 0x2b2b2f)
        ...
        applySmoothRoundedCorners(doneButton.layer)
    ...
    func animateIn() {
        ...
        if case .regular = containerLayout?.0.metrics.widthClass {
            isTablet = true
        }
        ...
        if isTablet {
            ...
            path.addEllipse(in: CGRect(origin: .zero, size: fromRect.size))
            maskLayer.path = path
            ...
            transition.updateTransformScale(layer: maskLayer, scale: maxRadius * 2.0 / fromRect.width) { [weak self] _ in
                self?.layer.mask = nil
            }
        }
    ...
    func animateOut(completion: (() -> Void)? = nil) {
        ...
        contentBackgroundNode.backgroundColor = .clear
        let toRect = requestEndFrame?() ?? .zero

        if isTablet || toRect.equalTo(.zero) || isCancelPressed {
            ...
            let transition: ContainedViewLayoutTransition = .animated(duration: 0.5, curve: .spring)
            let fullWidth = containerLayout?.0.size.width ?? layer.bounds.width

            transition.updateAlpha(node: cancelButton, alpha: 0.0)
            ...
            transition.updatePosition(node: contentContainerNode, position: CGPoint(x: toRect.midX, y: toRect.midY))
            transition.updateTransformScale(node: contentContainerNode, scale: toRect.width / fullWidth) { [weak self] _ in
                self?.dismiss?()
                completion?()
            }
        }
    ...
    func containerLayoutUpdated(_ layout: ContainerViewLayout, navigationBarHeight: CGFloat, transition: ContainedViewLayoutTransition) {
        ...
        let statusBarInsets = layout.insets(options: .statusBar)

        let titleSize = self.titleNode.measure(CGSize(width: contentFrame.width, height: .greatestFiniteMagnitude))
        let titleFrame = CGRect(origin: CGPoint(x: floor((contentFrame.width - titleSize.width) / 2.0), y: statusBarInsets.top + (44.0 - titleSize.height) / 2.0), size: titleSize)
        ...
        let cancelButtonFrame = CGRect(origin: CGPoint(x: previewFrame.minX + 17.0, y: statusBarInsets.top + (44.0 - cancelButtonSize.height) / 2.0), size: cancelButtonSize)
        ...
        let buttonBottomInset: CGFloat = 14.0
        ...
        let doneButtonFrame = CGRect(x: floorToScreenPixels((contentFrame.width - buttonWidth) / 2.0), y: previewFrame.maxY - doneButtonHeight - buttonBottomInset - layout.intrinsicInsets.bottom, width: buttonWidth, height: doneButtonHeight)
...
private class WheelControlNode: ASDisplayNode, UIGestureRecognizerDelegate {
    ...
    public init(items: [Item], selectedIndex: Int) {
        ...
        self.itemNodes = items.map { item in
            ...
            itemNode.setTitle(item.title.uppercased(), with: selectedTextFont, with: .white.withAlphaComponent(0.5), for: .normal)
        ...
        if !self.itemNodes.isEmpty {
            ...
            for i in 0 ..< self.itemNodes.count {
                ...
                if itemNode.isSelected != isSelected {
                    ...
                    itemNode.setTitle(title, with: selectedTextFont, with: isSelected ? .white : .white.withAlphaComponent(0.5), for: .normal)
                    if isSelected {
