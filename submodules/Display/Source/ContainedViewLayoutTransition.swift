...
public extension ContainedViewLayoutTransition {
    ...
    func updateTransformRotation(layer: CALayer, angle: CGFloat, beginWithCurrentState: Bool = false, completion: ((Bool) -> Void)? = nil) {
        let t = layer.transform
        let currentAngle = atan2(t.m12, t.m11)
        if currentAngle.isEqual(to: angle) {
            if let completion = completion {
        ...
        case let .animated(duration, curve):
            ...
            layer.transform = CATransform3DMakeRotation(angle, 0.0, 0.0, 1.0)
            layer.animateRotation(from: previousAngle, to: angle, duration: duration, timingFunction: curve.timingFunction, mediaTimingFunction: curve.mediaTimingFunction, completion: { result in
                if let completion = completion {
                    completion(result)
                }
            })
